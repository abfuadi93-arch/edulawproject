...
<div {{ $attributes->merge(['class' => 'rounded-2xl bg-[#07111F] p-5 text-white shadow-sm shadow-brand-ink/10']) }}>
    <p class="text-[11px] font-bold uppercase tracking-[0.18em] text-[#C9891F]">
        {{ $eyebrow }}
    </p>
    ...
    <a
        href="{{ $url }}"
        class="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[#B87316] px-4 py-3 text-sm font-bold text-white transition duration-300 hover:bg-[#D99A25] hover:text-white"
    >
        {{ $label }}
