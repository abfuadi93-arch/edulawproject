...
<section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8" aria-labelledby="event-details-title">
    <p class="text-xs font-bold uppercase tracking-[0.22em] text-brand-teal">Informasi Acara</p>
    <h2 id="event-details-title" class="mt-3 text-2xl font-bold tracking-tight text-brand-navy">Jadwal & lokasi</h2>
    ...
    <dl class="mt-6 grid gap-6 sm:grid-cols-2">
        <div>
            ...
            <dd class="mt-2 font-bold leading-7 text-brand-navy">
        ...
        @if ($program->eventDateValue(true))
            <div>
                ...
                <dd class="mt-2 font-bold leading-7 text-brand-navy"><time datetime="{{ $program->eventDateValue(true) }}">{{ $program->eventDateLabel(true) }}</time></dd>
            </div>
        @endif
        @if ($program->location || $program->online_url || $program->venue_address_label)
            <div class="min-w-0">
                ...
                <dd class="mt-2 leading-7 text-slate-600">
                    @if ($program->location)<p class="font-bold text-brand-navy">{{ $program->location }}</p>@endif
