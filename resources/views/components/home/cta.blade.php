    ...
    <div class="section-shell">
        <div class="relative overflow-hidden rounded-2xl bg-[linear-gradient(105deg,#173b68,#245485_65%,#2c5273)] shadow-[0_24px_70px_-35px_rgba(15,23,42,0.6)]">
            ...
            <div class="grid gap-7 px-6 py-9 text-center sm:px-10 lg:grid-cols-[1fr_auto] lg:items-center lg:px-12 lg:py-11 lg:text-left">
                <div>
                    <p class="home-section-eyebrow text-[#f0c55e]">{{ $eyebrow }}</p>
                    <h2 class="home-section-title max-w-2xl text-white">{{ $title }}</h2>
                    <p class="mt-3 max-w-3xl text-[15px] leading-7 text-slate-200">{{ $body }}</p>
                </div>
                <div class="flex flex-col gap-3">
                    <a href="{{ $primaryUrl }}" class="inline-flex min-w-48 min-h-11 items-center justify-center gap-3 rounded-lg bg-[#f8bd38] px-6 py-3 text-[13px] font-extrabold text-[#142f57] shadow-sm transition hover:bg-[#ffd263]">{{ $primaryLabel }} <span aria-hidden="true">→</span></a>
                    <a href="{{ $secondaryUrl }}" class="inline-flex min-w-48 min-h-11 items-center justify-center rounded-lg border border-white/20 bg-white/10 px-6 py-3 text-[13px] font-extrabold text-white transition hover:bg-white/15">{{ $secondaryLabel }} →</a>
                </div>
            </div>
        </div>
    </div>
