...
<main class="overflow-x-clip bg-[#f7f8fa] text-brand-ink">
    ...
    >
        <p class="font-display text-7xl font-bold leading-none tracking-[-0.06em] text-brand-amber sm:text-8xl" aria-hidden="true">404</p>
    </x-shared.page-header>
    ...
    <section class="py-9 sm:py-10 lg:py-11">
        <div class="section-shell grid gap-5 lg:grid-cols-[0.42fr_0.58fr] lg:items-stretch">
            <div class="flex flex-col justify-between rounded-[14px] bg-white p-6 sm:p-7">
                <div>
                    <p class="text-[10px] font-bold uppercase tracking-[0.18em] text-brand-teal">Arah Berikutnya</p>
                    <h2 class="mt-2 font-display text-2xl font-bold leading-tight text-brand-navy sm:text-3xl">Mari kembali ke ruang pengetahuan Edulaw.</h2>
                ...
                <div class="mt-6 flex flex-col gap-3 sm:flex-row">
                    <a href="{{ route('home') }}" class="inline-flex min-h-11 items-center justify-center gap-2 rounded-lg bg-brand-navy px-5 py-3 text-sm font-bold text-white transition hover:bg-[#294f82]">
                        Kembali ke Beranda <span aria-hidden="true">→</span>
                    </a>
                    <a href="{{ route('search.index') }}" class="inline-flex min-h-11 items-center justify-center gap-2 rounded-lg border border-slate-200 bg-white px-5 py-3 text-sm font-bold text-brand-navy transition hover:border-brand-teal/40 hover:bg-brand-teal-soft">
                        Cari Konten
                    </a>
                </div>
            </div>
            ...
            <nav class="rounded-[14px] bg-brand-navy p-5 sm:p-6" aria-label="Kanal utama Edulaw">
                <p class="text-[10px] font-bold uppercase tracking-[0.18em] text-brand-amber">Jelajahi Kanal</p>
                <div class="mt-4 grid gap-2 sm:grid-cols-2">
                    @foreach ($quickLinks as $link)
                        <a href="{{ $link['url'] }}" class="group rounded-[12px] bg-white/8 p-4 transition hover:bg-white/12">
                            <span class="flex items-center justify-between gap-4">
                                <span class="text-base font-bold text-white">{{ $link['title'] }}</span>
                ...
                <div class="mt-4 flex flex-col gap-2 border-t border-white/12 pt-4 text-sm text-white/70 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <a href="{{ route('contact.index') }}" class="font-bold text-white transition hover:text-brand-amber">Hubungi Edulaw →</a>
                </div>
            </nav>
        </div>
    </section>
